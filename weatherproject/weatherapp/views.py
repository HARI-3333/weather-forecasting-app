...
import requests #for api request upon another website
import datetime
...
def home(request):
    ...
    url = f'https://api.openweathermap.org/data/2.5/weather?q={city}&appid=b43f11130b9d81bf6e1ec5d161f4d36f'
    
    # Parameters to request temperature in metric units (Celsius)
    PARAMS = {'units': 'metric'}

    # Your Google Custom Search API Key (keep it secret)
    API_KEY = "AIzaSyCHy_fVo4BQvQHy_2F1Z73JECKNWLf_-NM"#BY ME ADD

    # Your Google Custom Search Engine ID
    SEARCH_ENGINE_ID = "167a5b78207cf4d32"#BY ME ADD
    ...
    data = requests.get(city_url).json() #.json just like library
    count = 1
    ...
    try:
        ...
        data = requests.get(url, params=PARAMS).json()#......all are covert into .json format 
